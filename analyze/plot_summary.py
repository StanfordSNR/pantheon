...
class PlotSummary:
    ...
    def plot_summary(self):
        ...
        max_queue_bound = max_outstanding_packets * 10

        median_loss_rate = numpy.median(all_loss_rates)
        max_loss_bound = median_loss_rate * 4
        max_loss_bound = max(max_loss_bound, .1)
        max_loss_bound = min(max_loss_bound, .5)

        print("real limits: median of (minum prop delay of a single run) %d, max throughput in any bin %.2f, median of (max thoughput in any bin) throughput %.2f, outstanding packets %d, median loss rate %.4f" % (median_min_delay, max_throughput, median_max_throughput, max_outstanding_packets, median_loss_rate))
