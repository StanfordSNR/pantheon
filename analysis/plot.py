...
class Plot(object):
    ...
    def cleanup_stats_log(self, stats_log_path):
        saved_lines = ''
        ...
        with open(stats_log_path) as stats_log:
            for line in stats_log:
                if 'Start at:' in line:
                    ...
                    saved_lines += line
                elif 'Worst absolute clock offset:' in line:
                    saved_lines += line
                else:
                    continue
    ...
    def update_stats_log(self, cc, run_id, stats):
        ...
        with open(stats_log_path) as stats_log:
            for line in stats_log:
                if 'Start at:' in line:
                    ...
                    ret = re.match(
                        r'Worst absolute clock offset: (.*?) ms', line)
        ...
        with open(stats_log_path, 'w') as stats_log:
            stats_log.write(saved_lines)
            stats_log.write('\n# Datalink statistics (generated by %s at %s)\n'
    ...
    def eval_performance(self):
        ...
        for cc in self.cc_schemes:
            for run_id in xrange(1, 1 + self.run_times):
                ...
                offset = self.update_stats_log(cc, run_id, stats)
