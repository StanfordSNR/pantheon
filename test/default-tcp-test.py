...
def main():
    ...
    port_proc = subprocess.Popen('./find-unused-port', cwd = path, 
                stdout = subprocess.PIPE, shell = True) 
    port = port_proc.communicate()[0]

    recv_cmd = 'python default-tcp.py receiver %s' % port
    recv_proc = subprocess.Popen(recv_cmd, cwd = path, shell = True,
                    stdout = DEVNULL, stderr = DEVNULL)

    ip = '127.0.0.1'
    send_cmd = 'python default-tcp.py sender %s %s < ../test/random-input' \
                % (ip, port)
